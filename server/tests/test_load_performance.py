...
class TestConcurrentCalls:
    ...
    async def test_concurrent_customer_lookups(self, db_session):
        ...
        tasks = [lookup_customer(db_session, f"+1555555{i:04d}") for i in range(10)]
...
class TestDatabaseConnectionPool:
    ...
    async def test_connection_pool_exhaustion(self, db_session):
        ...
        tasks = [lookup_customer(db_session, "+15555550001") for _ in range(50)]
        ...
        print(
            f"✓ 50 concurrent operations: {successes} succeeded, {failures} failed in {duration:.2f}s"
        )
...
class TestRedisPerformance:
    ...
    async def test_concurrent_session_operations(self):
        ...
        # Write sessions
        write_tasks = [set_session(call_sid, session_data) for call_sid in call_sids]
...
class TestAudioPipelinePerformance:
    ...
    async def test_stt_to_llm_latency(self, mock_openai, mock_tts, mock_stt):
        ...
        async def mock_get_transcript():
            ...
            return {"type": "final", "text": "I need to book an appointment"}
        ...
        async def mock_generate_response(stream=True):
            ...
            yield {"type": "content_delta", "text": "I can help you with that."}
...
class TestScalabilityLimits:
    ...
    async def test_maximum_concurrent_sessions(self):
        ...
        from app.services.redis_client import delete_session, set_session
        ...
        tasks = [
            set_session(
                f"CA_scale_test_{i}", {"customer_id": i, "state": "GREETING", "transcript": []}
            )
        ...
        # Cleanup
        cleanup_tasks = [delete_session(f"CA_scale_test_{i}") for i in range(num_sessions)]
