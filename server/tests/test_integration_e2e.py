...
class TestInboundCallFlowExistingCustomer:
    ...
    async def test_existing_customer_books_appointment(
        ...
        response = await client.post(
            ...
            data={
                "CallSid": "CA123test",
                "From": test_customer.phone_number,
                "To": "+15555550000"
            }
        )
    ...
    @pytest.mark.asyncio
    async def test_customer_lookup_during_call(
        ...
        response = await client.get(
            f"/api/v1/customers/lookup?phone={test_customer.phone_number}"
        )
...
class TestVoiceWebSocketFlow:
    ...
    async def test_audio_pipeline_integration(
        ...
        async def mock_get_transcript():
            return {
                "type": "final",
                "text": "I need to book an oil change"
            }
        ...
        async def mock_generate_response(stream=True):
            yield {
                "type": "content_delta",
                "text": "I can help you book an oil change. "
            }
            yield {
                "type": "content_delta",
...
class TestCRMToolsIntegration:
    ...
    async def test_cancel_appointment_tool(
        self, db_session, test_appointment: Appointment
    ):
...
class TestPerformanceTargets:
    ...
    @pytest.mark.asyncio
    async def test_customer_lookup_latency(
        self, db_session, test_customer: Customer
    ):
